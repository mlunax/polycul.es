...
'''.format(self.graph_hash)
    ...
    def as_dot(self, edge_labels=False):
        ...
        for edge in parsed['links']:
            ...
            if edge_labels:
                dot += ',label="{label}"'.format(
                    label=edge.get('centerText', ''))
            if 'dashed' in edge:
                dot += ',style=dashed'
    ...
    def as_svg(self, edge_labels=False, include_style=False, embed=False,
        ...
        for edge in parsed['links']:
            ...
            """.format(
                ...
                y2=edge['target']['y'],
                width=edge['strength'],
                dasharray=' stroke-dasharray="{s} {s}"'.format(
                    s=edge['strength']) if 'dashed' in edge else '')
        return svg.format(
    ...
    def as_png_from_svg(self, edge_labels=False, include_style=False,
                        embed=False):
        source = tempfile.NamedTemporaryFile()
        dest = tempfile.NamedTemporaryFile()
        source.write(self.as_svg(
        ...
        subprocess.check_call([
            'convert',
            'svg:{}'.format(source.name),
            'png:{}'.format(dest.name)
        ])
        png = dest.read()
        source.close()
        dest.close()
